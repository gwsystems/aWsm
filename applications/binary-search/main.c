...
uint32_t find(int* haystack, uint32_t haystackLength, int32_t needle) {
    assert(haystackLength <= bufferCapacity);
...
int main(int argc, char* argv[]) {
    ...
    assert(find(first, FIRST_LEN, 6) == 0);
    assert(find(second, SECOND_LEN, 6) == 3);
    assert(find(second, SECOND_LEN, 1) == 0);
    assert(find(second, SECOND_LEN, 11) == 6);
    assert(find(third, THIRD_LEN, 144) == 9);
    assert(find(fourth, FOURTH_LEN, 21) == 5);
    assert(find(fifth, FIFTH_LEN, 0) == -1);
    assert(find(fifth, FIFTH_LEN, 13) == -1);
    assert(find(sixth, SIXTH_LEN, 1) == -1);
    assert(find(seventh, SEVENTH_LEN, 0) == -1);
}
