...
__attribute__((noreturn)) __attribute__((always_inline)) void
awsm_abi__trap_unreachable()
{
...
INLINE i32
instruction_memory_grow(i32 count)
{
...
clear_table()
{
	for (int i = 0; i < INDIRECT_TABLE_SIZE; i++) { indirect_table[i] = (struct indirect_table_entry){0}; }
}
...
i32_rem(i32 a, i32 b)
{
	...
	if (unlikely(a == INT32_MIN)) { return (a + abs(b)) % b; }
...
i64_rem(i64 a, i64 b)
{
	...
	/* Because MIN is one less than MAX, we can FPE here */
	if (unlikely(a == INT64_MIN)) { return (a + labs(b)) % b; }
