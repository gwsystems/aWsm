...
wasi_context_init(wasi_options_t *options)
{
	...
	for (int i = 1; i < options->argc; i++) { init_options.argv[i] = options->argv[i]; }
...
__wasi_errno_t
wasi_snapshot_preview1_backing_args_get(void *wasi_context, char **argv, char *argv_buf)
{
...
__wasi_errno_t
wasi_snapshot_preview1_backing_environ_get(void *wasi_context, char **environ, char *environ_buf)
{
...
__wasi_errno_t
wasi_snapshot_preview1_backing_fd_seek(void *wasi_context, __wasi_fd_t fd, __wasi_filedelta_t offset,
...
__wasi_errno_t
wasi_snapshot_preview1_backing_fd_tell(void *wasi_context, __wasi_fd_t fd, __wasi_filesize_t *retptr0)
{
...
{
	return uvwasi_path_filestat_get((uvwasi_t *)wasi_context, fd, flags, path, path_len,
	                                (uvwasi_filestat_t *)retptr0);
}
...
{
	return wasi_snapshot_preview1_backing_path_filestat_set_times((uvwasi_t *)wasi_context, fd, flags, path,
...
{
	return uvwasi_poll_oneoff((uvwasi_t *)wasi_context, (uvwasi_subscription_t *)in, (uvwasi_event_t *)out,
	                          nsubscriptions, retptr0);
}

_Noreturn void
wasi_snapshot_preview1_backing_proc_exit(void *wasi_context, __wasi_exitcode_t rval)
{
